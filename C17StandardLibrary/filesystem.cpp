...
void UsingPath(std::string_view path)
{
...
void TraversingDirectory(std::string_view file)
{
    ...
    for (const auto &dir_entry : dir_entries)
    {
        ...
        case fs::file_type::regular:
            std::cout << "[FILE]\t" << p.string() << '\t' << dir_entry.file_size() << std::endl;
            break;
        }
    }
}
...
{
    std::string selectedPath{R"(/home/abdallah/Documents/c++new/C17StandardLibrary/newfile)"};

    // UsingPath(selectedPath);
    // DirectoryOperations(selectedPath);
    // TraversingDirectory(selectedPath);
